...
# posttestdata=pd.read_csv('F:\热爱学习\jupyter\量化交易\配对交易\\result\SH\k0.05\\tf1tt1\post\\600012.SHtf1tt1k0.05.csv')
# posttestdata=posttestdata.iloc[:,1:]
# posttestdata
# pretestdata=pd.read_csv('F:\热爱学习\jupyter\量化交易\配对交易\\result\SH\k0.05\\tf1tt1\pre\\600012.SHtf1tt1k0.05.csv')
# pretestdata=pretestdata.iloc[:,1:]
# pretestdata

# preStartTime='2006-04-01'
# preEndTime='2014-06-01'
# postStartTime='2015-06-01'
...
def cutDataByTime(data,StartTime,EndTime,tf,tt):
    ...
    dataCopy=data.copy()
    dataCopy.columns = dataCopy.columns.str[:19]
    dataCopy.columns = pd.to_datetime(dataCopy.columns)
    StartTime = datetime.datetime.strptime(StartTime, "%Y-%m-%d")
    EndTime = datetime.datetime.strptime(EndTime, "%Y-%m-%d")
    EndTime = EndTime - relativedelta(months=tt) - relativedelta(months=tf)
    dataCopy=dataCopy[dataCopy.columns[(dataCopy.columns >= StartTime) & (dataCopy.columns <= EndTime)]]
    return dataCopy
#%%

#根据时间切片计算汇总结果
def outputSummaryByTime(IDSet, IsSH):
        ...
        #     for tt in range(1, 3):
                ...
                for AID in IDSet:
                    ...
                    if preDataPath.exists():
                        ...
                        if preData.size>0:
                            preNameLi.append(AID)
                            preYieldRateLi.append(preData.iloc[-4].mean()/tt)
                            preExcessYieldRateLi.append(preData.iloc[-1].mean()/tt)
                            preOpeningTimesLi.append(preData.iloc[-3].mean()/tt)
                            preClosingTimesLi.append(preData.iloc[-2].mean()/tt)
                            preSharpeRateLi.append((preData.iloc[-1].mean()/tt)/ (preData.iloc[-4].std()/tt))
                    if postDataPath.exists():
                        postData=pd.read_csv(postDataPath)
                        postData = postData.iloc[:, 1:]
                        ...
                        if postData.size>0:
                            postNameLi.append(AID)
                            postYieldRateLi.append(postData.iloc[-4].mean()/tt)
                            postExcessYieldRateLi.append(postData.iloc[-1].mean()/tt)
                            postOpeningTimesLi.append(postData.iloc[-3].mean()/tt)
                            postClosingTimesLi.append(postData.iloc[-2].mean()/tt)
                            postSharpeRateLi.append((postData.iloc[-1].mean()/tt)/ (postData.iloc[-4].std()/tt))
                KpreNameLi.append('tf' + str(tf) + 'tt' + str(tt))
                KpreYieldRateLi.append(np.mean(preYieldRateLi))
                KpreOpeningTimesLi.append(np.mean(preOpeningTimesLi))
                KpreClosingTimesLi.append(np.mean(preClosingTimesLi))
...
outputSummaryByTime(SHStockCode, 1)
outputSummaryByTime(SZStockCode, 0)